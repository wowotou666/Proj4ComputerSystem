...
//sets are made up of cache lines and cache is made up of sets
...
void initCache()
{
    S = pow(2,s);
    B = pow(2,b);
      //allcote space for S * size of one set
      cache = malloc(S * sizeof(cache_set_t)); 

    for(int i = 0; i < S; i++) { 
      //allcote space for E * size of one line
...
void freeCache()
{
  ...
  //free cache after all sets are freeed
  free(cache); 
}
...
{
  //set the addresses of tag and set
  mem_addr_t tag = addr >> (b + s);
  mem_addr_t set = (addr >> b) & (S - 1);

  //declare two variables, which are most and least values of used items from lru
  int MRU = cache[set][0].lru;   
  int LRU = cache[set][0].lru;   

  //loop through each line in sets
  for(int i = 0; i < E; i++) { 
    //search for the most recently used line - MRU
    if(MRU < cache[set][i].lru){      
      MRU = cache[set][i].lru;
    }
  }
  ...
  //loop through each line in sets
  for(int i = 0; i < E; i++) { 
    //when line is valid and tag matches
    if( cache[set][i].valid == 1 && cache[set][i].tag == tag ){  
      //there exists a hit and update
      cache[set][i].lru = MRU + 1;
      hit_count++;
      return;
    }
  }
  
    //now there exists a miss
    for(int i = 0; i < E; i++) { 
      //when line is not valid
      if(cache[set][i].valid == 0 ){ 
        //set tag, lru and valid
        cache[set][i].tag = tag;
        cache[set][i].lru = MRU + 1;   
        cache[set][i].valid = 1;
        miss_count++;
    ...
    //there is an eviction, find LRU line and update it
    for (int i = 0; i < E; i++) {
      if(cache[set][i].lru <= LRU && cache[set][i].valid == 1 ){
        LRU = cache[set][i].lru;
      }
    }
...
/* TODO4 - FILL IN THE MISSING CODE
